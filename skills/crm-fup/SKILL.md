...
3. Classificar o novo Status:
   ...
   **Erro fácil de cometer**: "Esperar"/"FUP" exigem que o contato tenha respondido de verdade em ALGUM momento da conversa — não basta você ter mandado um segundo template numa data diferente. Contato que nunca respondeu nada, mesmo depois de 2+ tentativas em dias distintos, é Ghost 2, nunca "Esperar". O critério é "existiu diálogo real dele", não "eu já tentei mais de uma vez".
...
- **Contato renomeado pra cliente** (perdeu o marcador de busca do nome ao converter): some da busca, a linha da FUP fica congelada até alguém notar. Sem detecção automática ainda, mas o procedimento pra tratar já existe: ver `crm-lead/SKILL.md`, seção "Lead que converteu sozinho".
...
- **CRÍTICO — lead converteu organicamente (status de compra virou positivo sem responder o WhatsApp nem qualquer contato do usuário), achado durante a leitura de uma conversa**: escrever na mesma resposta, mesma regra do item abaixo. `Status` FUP vira `Comprou`, `Observação` ganha o contexto (data, como foi reimpactado). Se o contato ainda tiver o marcador de "lead", renomear pro marcador de cliente no Google Contatos (`contacts_client.py`, `updatePersonFields='names'`, escrever em `givenName`) e sincronizar na aba de leads: status de compra em TODAS as linhas do grupo de duplicata (não só na âncora, senão formatação condicional que destaca quem comprou não pinta as demais), demais campos de contexto só na linha âncora. Não esperar rodar o skill de captação de novo pra fechar o ciclo.
- **CRÍTICO — contato relatado pelo usuário fora do WhatsApp (e-mail, telefone, pessoalmente) precisa ser escrito na Sheet NA MESMA resposta, nunca só reconhecido no chat.** Esse tipo de fato nunca entra sozinho por nenhum fluxo automático (este skill só lê WhatsApp; o skill de captação só lê a Sheet) — se não for escrito na hora, ele se perde e a linha fica presa em estado incompleto indefinidamente, mesmo com prova já enviada pelo usuário. Ao ouvir "contatei fulano por [canal]", escrever imediatamente via `sheets_client`/`update_cell` (nunca esperar terminar de processar o resto da conversa): a Sheet de leads ganha o fato (canal + data se souber), a FUP ganha `Quem mandou por último` + `Observação` explicando o canal, mesmo sem rodar o resto do fluxo de captura/diff do WhatsApp.
