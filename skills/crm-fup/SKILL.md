...
- **CRÍTICO — lead converteu organicamente (status de compra virou positivo sem responder o WhatsApp nem qualquer contato do usuário), achado durante a leitura de uma conversa**: escrever na mesma resposta, mesma regra do item abaixo. `Status` FUP vira `Comprou`, `Observação` ganha o contexto (data, como foi reimpactado). Se o contato ainda tiver o marcador de "lead", renomear pro marcador de cliente no Google Contatos (`contacts_client.py`, `updatePersonFields='names'`, escrever em `givenName`) e sincronizar o status de compra na aba de leads (linha âncora se for duplicata). Não esperar rodar o skill de captação de novo pra fechar o ciclo.
